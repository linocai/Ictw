...
- 写作链四 Agent：Memory Selector → Writer → Checker → 用户接受 → Extractor。Reviser 已移除，语义契约见 PROJECT_PLAN.md。
...
- 字数按去空白字符数计算，只要求正文不少于 4000 字且正常结束，不设产品上限；首次长度／截断失败最多从同一输入完整重写一次，常量定义在 `Backend/app/services/context.py`。
