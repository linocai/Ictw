...
## [ERR-20260802-031] macOS iconv rejected stdout redirection during UTF-8 validation

**Logged**: 2026-08-02T17:52:06+08:00
**Priority**: low
**Status**: resolved
**Area**: tests
...
The first validation pass used macOS `iconv` with output redirected to `/dev/null`; it reported an inappropriate ioctl. A retry with GNU-style `-o` also failed because this host's BSD `iconv` does not support that option. Both exported TXT files remained readable and their section counts matched.

### Error

`iconv: iconv(): Inappropriate ioctl for device`

`iconv: invalid option -- o`

### Suggested Fix

For artifact validation on this host, avoid relying on `iconv` output behavior and GNU-only flags. Use strict `Encode::decode(..., FB_CROAK)` plus `file -I` to validate UTF-8 without reproducing file contents in logs.
...
- **Resolved**: 2026-08-02T17:52:06+08:00
- **Notes**: Switched to strict Perl UTF-8 decoding and `file -I`; both files passed and were identified as `text/plain; charset=utf-8`.

---

## [ERR-20260804-001] Parallel Xcode targets shared one build database

**Logged**: 2026-08-04T17:23:20+08:00
**Priority**: low
**Status**: resolved
**Area**: tests

### Summary
Running the iOS and macOS `xcodebuild` validations in parallel against the default DerivedData directory locked the shared XCBuild database.

### Error
`accessing build database .../XCBuildData/build.db: database is locked`

### Context
- Both schemes belong to `App/LinoI.xcodeproj` and default to the same DerivedData location.
- The iOS build succeeded; only the overlapping macOS invocation failed.
- Rerunning the macOS build after the iOS build completed succeeded without code changes.

### Suggested Fix
Run the two target validations serially, or give each concurrent build a distinct `-derivedDataPath`.
...
### Resolution
- **Resolved**: 2026-08-04T17:23:20+08:00
- **Notes**: Reran the macOS target after the shared build database was released; build succeeded.

---

## [ERR-20260802-030] GitHub release verification requested an unsupported JSON field

**Logged**: 2026-08-02T17:46:37+08:00
**Priority**: low
**Status**: resolved
**Area**: infra
...
The first Build 22 release verification requested `isLatest` from `gh release view`, but that field is not exposed by the installed GitHub CLI version.
...
Verify published/draft/prerelease state with supported `gh release view` fields, and verify “Latest” separately through `gh release list` when needed.
...
- **Resolved**: 2026-08-02T17:46:37+08:00
- **Notes**: Reran the final check using only supported fields and a separate release-list query.

---

## [ERR-20260802-029] Reviewer spawn included an unavailable model override

**Logged**: 2026-08-02T17:38:41+08:00
**Priority**: low
**Status**: resolved
**Area**: config
...
The first independent Reviewer spawn explicitly requested `gpt-5.5`, but the current spawn interface only accepts its listed model overrides and rejected the call.

### Error

`Unknown model gpt-5.5 for spawn_agent. Available models: gpt-5.6-sol, gpt-5.6-terra`

### Suggested Fix

When selecting the dedicated `reviewer` agent type, omit model and reasoning overrides so the role can use its configured defaults.
...
- **Notes**: Retried with `agent_type: reviewer`, `fork_turns: none`, and no model override; the review started successfully.

---

## [ERR-20260802-028] Local visual QA seeded the wrong API prefix

**Logged**: 2026-08-02T17:36:00+08:00
**Priority**: low
**Status**: resolved
**Area**: tests
...
The first local shelf-layout QA seed request used `/api/books` instead of the configured `/api/v1/books` route and returned HTTP 404.
...
- The disposable backend started successfully on `127.0.0.1:18080`.
- No product or production data was changed; only the first temporary seed request failed.

### Suggested Fix

Read `Settings.api_prefix` before constructing local QA requests, and use `/api/v1` consistently with `LinoAPI`.

### Metadata

- Reproducible: yes
- Related Files: `Backend/app/config.py`, `App/LinoI/LinoAPI.swift`

### Resolution

- **Resolved**: 2026-08-02T17:36:00+08:00
- **Notes**: Corrected the disposable QA request to `/api/v1/books` before creating any test data.

---

## [ERR-20260802-027] Planner role cannot use a full-history fork override

**Logged**: 2026-08-02T15:30:00+08:00
**Priority**: low
**Status**: resolved
**Area**: infra
...
Spawning an explicit `planner` role with `fork_turns: "all"` was rejected because full-history forks inherit the parent agent type.
...
Use `fork_turns: "none"` for an explicit Planner and provide all repository paths, scope constraints, and required deliverables in the task message.

### Resolution

Retried with an independent Planner context and an explicit self-contained v1.7.0 brief.
