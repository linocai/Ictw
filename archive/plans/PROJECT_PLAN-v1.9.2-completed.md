# ICTW PROJECT_PLAN — v1.9.2 completed
...
- **问题**：当前可编辑 Agent 人格是全局配置，但叙事人称、第一人称叙述者、文风、关系推进速度等要求属于具体书籍。把“本书采用第一人称”写进全局灵感创造师提示词会错误影响其他小说。
- **目标**：所有 Agent 的可编辑提示词支持“全局默认 + 单书覆盖”。每本书、每个 Agent 可分别选择“跟随全局”或“本书自定义”；未配置覆盖时始终继承当前全局值，删除覆盖后立即恢复跟随全局。
- **生效边界**：书级内容只覆盖可编辑人格／创作要求；不可编辑的 `PROGRAM_PROTOCOLS`、人物白名单、字数与来源校验、Writer／Extractor 生命周期等程序规则继续强制生效。模型 Profile、binding、thinking／effort 与 temperature 暂时保持全局，不随本功能下沉到单书。
- **前端方向**：入口放在书籍设置的 Agent 区域，逐 Agent 显示继承状态、本书自定义编辑框与“恢复跟随全局”；不在每章或每次生成界面重复增加配置。灵感创造师可在这里写“本书采用第一人称，‘我’指某人物，不得进入‘我’无法感知的他人内心”等书级视角约束。
- **后端方向**：以 `(book_id, agent_role)` 保存可选覆盖，并由统一解析器生成有效人格；各 Agent 不得自行拼接或绕开继承规则。删除 Book 时级联删除覆盖；公开设置 API 必须同时返回全局默认、书级覆盖和最终生效值，避免客户端猜测。
- **验收重点**：同一全局 Agent 下，两本书可分别使用第一／第三人称且互不串扰；修改全局提示词只影响仍在“跟随全局”的书；书级自定义不能覆盖程序协议；旧书在无覆盖数据时行为与升级前一致。
- **版本**：不进入 `v1.9.2(39)` 快修，留待后续正式版本单独规划、迁移、双端施工与发布。
