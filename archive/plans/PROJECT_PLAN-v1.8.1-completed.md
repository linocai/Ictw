...
> 唯一现行计划与状态来源；与 `archive/` 冲突时以本文为准。`v1.8.1(32)` 宽松校验快修已完成发布；用户确认的四章重提已执行，1 章新激活、3 章安全保留 legacy；macOS 图标已与 iOS 统一并替换发布包；章节人物关联保存 500 已作 Backend-only 快修。2026-08-07 已完成香港→宁波生产迁移，当前公网入口为 `https://ictw.linotsai.top`。
...
- 云迁移已完成：新机以 `linoi` 非登录服务账户运行 `/opt/linoi/backend`，仅监听 Docker 网桥 `172.18.0.1:8787`；Nginx Proxy Manager 为 `ictw.linotsai.top` 提供 HTTPS 反代，HTTP 308 跳转 HTTPS。最终一致性停服备份 `/opt/linoi/backups/20260807-ningbo-cutover.eyu6o0`，数据库 SHA-256 `56b24e725ae779d6c3be3b422e8c33cb064d0d92999dfba6634ca0f325977843`；新旧均为 `3 books / 68 chapters / 26 characters`，Alembic `20260805_0010`、SQLite integrity／foreign keys、带鉴权公网健康 `1.8.1` 与 `NRestarts=0` 均通过。旧机服务已 stopped + disabled 并保留备份；未改代码、数据库结构、版本、Build、tag 或 Release。已安装的客户端需在连接设置中改为新地址。
