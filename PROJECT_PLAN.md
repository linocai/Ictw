...
- **v1.4.0「前端视觉升级」施工中**（纯前端，Backend/ 零改动、无迁移、无部署；后端健康仍报 `1.3.2` 属预期）。五块：Motion/视觉 token 基建 + 锁浅色 → macOS 动画平滑 → iOS 动画平滑 + 阅读三主题 + 字族/控件一致性 → 视觉 bug 扫尾 → 验收发版。双 target 抬 `1.4.0(12)`。**块①②③④已完成**（块① token 基建 + 双端锁浅色；块② macOS 全站切换点 transition/matchedGeometry；块③ iOS 动画同构 + 阅读页三主题 + 字族/控件一致性；块④ 窄窗抽屉互斥+scrim、人物 chip 截断、阅读 night 主题 ProgressView/NSTextView 选中色、Mac 章节空态，见变更日志）；块②③的部分 GUI 像素级验收因环境限制未能完全完成，留待块⑤或用户空闲时补齐（见变更日志）；iOS night 状态栏灰缝确认为系统级限制、无 app 层干净解法（块④已实测两种方案均无效，见变更日志）；块⑤ 待做。详见「当前 Plan」。
