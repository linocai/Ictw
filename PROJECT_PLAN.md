...
- `v1.7.2(29)` Backend Extractor 质量收敛快修已完成：生产最近五次归档均出现人物事件／状态降级，其中一章对单个人物生成 15 条流水账式事件；确认并精确迁移现网旧版“200 字梗概／动态字段”人格，Extractor 实际温度从 0.3 降至 0.1。schema、提示词和确定性验证现共同限制人物事件按重要性排序、每人最多 3 条、本章最多 8 条、同一人物同一事件不得重复；超限／重复项逐条保守丢弃并提示，不新增模型轮次，不放松证据或白名单。115 项后端测试、compileall、Alembic head、数据库完整性、内外网健康与零错误日志均通过；实现提交 `e579ebc`，生产备份 `/opt/linoi/backups/20260805-150015`。无 migration、版本、Build、客户端、App 包、tag 或 Release 变化，本补丁仅推 main。
