...
- **v1.4.0「前端视觉升级」施工中**（纯前端，Backend/ 零改动、无迁移、无部署；后端健康仍报 `1.3.2` 属预期）。五块：Motion/视觉 token 基建 + 锁浅色 → macOS 动画平滑 → iOS 动画平滑 + 阅读三主题 + 字族/控件一致性 → 视觉 bug 扫尾 → 验收发版。双 target 抬 `1.4.0(12)`。**块①②已完成**（块① token 基建 + 双端锁浅色；块② macOS 全站切换点 transition/matchedGeometry，见变更日志）；块②的 GUI 像素级验收因环境限制未能完成，留待块⑤或用户空闲时补齐（见变更日志）；块③-⑤ 待做。详见「当前 Plan」。
