...
- `v1.7.2(29)` Backend 人物事件逐条降级补丁施工中：生产实测确认 DeepSeek Flash／Pro 在定向重提时仍会重复输出同一类错误归属证据。保持事件类型、人物姓名、白名单与原文证据规则不变，改为后端逐条执行同一确定性校验；合格事件正常归档，不合格事件直接丢弃并在完成 Job 中记录数量和中文原因，不再阻塞 headline、摘要、章节记忆和人物状态。本补丁不改市场版本、Build、客户端或数据库结构。
