...
- 已发布基线：`v1.7.0(22)`（2026-08-02）；Build 22 修复 iOS 书架“新建书”卡片在两列网格换行时被固有尺寸压缩的问题，Backend、Alembic 与 macOS App 维持 `v1.6.5(20)`，Alembic head 仍为 `20260801_0008`。
- 当前状态：v1.7.0 阶段 0–6 全部完成；LinoI target 的市场版本保持 `1.7.0`、Build 升至 `22`，正式 iOS Archive、本机开发签名 IPA、`v1.7.0-build22` tag 与 GitHub Release 均已完成，无进行中的版本施工。
...
- 阶段 6：书架“新建书”卡片把 3:4 比例和列宽约束移到整个 Button 网格项，恢复完整书封尺寸与整卡点击热区；1／2／3 本书 UI 几何和点击回归、独立 Review、Build 22 Archive／IPA 与发布均完成。
...
`App/LinoI.xcodeproj/project.pbxproj` 在施工阶段不改；仅在所有验收通过且用户另行授权发包时，才可只改 **LinoI target** 的 Debug/Release `MARKETING_VERSION=1.7.0`、`CURRENT_PROJECT_VERSION=22`。不得触碰 LinoIMac 配置或任何 scheme。
...
| 6. Build 22 热修（已完成） | `ShelfViews.swift` 固定新建卡网格尺寸；市场版本保持 `1.7.0`，仅 Build 升至 `22`，生成独立 Archive／IPA 并发布 `v1.7.0-build22`。 | 1／2／3 本书 UI 几何与点击回归、独立 Review、客户端状态测试、iOS Debug／Release 均通过；不覆盖 Build 21 产物。 |
...
- `v1.7.0(21)`：iOS 视觉施工、全状态逐屏验收、全新 App Icon、正式 Archive、本机开发签名 IPA、tag 与 GitHub Release 已完成。
- `v1.7.0(22)`：书架新建卡布局热修、独立 Review、正式 Archive、本机开发签名 IPA、`v1.7.0-build22` tag 与 GitHub Release 已完成；Backend 与 macOS 不在本次热修范围。
