...
- 生产迁移禁止两台服务器同时接受写入。
- 正文按去空白字符数计算，只要求不少于 4000 字且正常结束，不设产品上限；首次长度或截断失败最多从同一输入完整重写一次。该门槛约束 Writer 产出；作者自带的短稿仍可接受，但必须显式声明忽略，不得由服务端默认放行。
- 人物选择是本章允许出现人物集合的上限；历史记忆和 Extractor 输出不会自动授权未选人物。
- `accept` 必须自行跑一遍确定性校验，不得因为该章从未产生过 candidate 就整段跳过。未选／重名人物属于正确性失败，任何忽略参数都不得放行，出路是把人物加进本章或加入豁免名单；长度类属于作者对自己稿件的判断，可由显式忽略通过。
- Writer 候选仅在后端留档；公开 API 和双端不得提供候选列表、选择接口或候选全文，被拒候选的逐字引证（`draft_evidence` / `bible_evidence`）同样不得出网，出网的只有 `kind` 与 `reason`。作者当前正文对应的 `visible_checker_result` 不在此列。
...
- 当前源码版本标记为 `v2.1.1(58)`（Writer／Checker定稿提示词后端已部署，256项通过／12项跳过；本轮未构建或换装客户端）。公开 Release及本机已安装macOS为 `v2.1.1(57)`（`ee09faf`，2026-09-22）；宁波Backend为 `v2.1.1`（`f29d823`，Build58后端），Alembic head `20260830_0013`。双端OS27 Release签名构建通过，macOS成稿页、长章节列表滚动、失败原因及同源码隔离Debug编辑页通过；iOS最终安装由用户在Xcode操作，未完成最终真机页面验收、未导出IPA。两台当前手机OS27开发支持现场核验通过，无pending设备。发布与资源证据见 `archive/plans/v2.1.1-error-visibility-plan.md`。
