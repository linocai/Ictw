...
- 双端／Backend `v1.8.0(31)` 已于 2026-08-05 完成 Extractor 合同漂移快修：模型事实引用只需本次输出内唯一，后端按数组顺序归一化为 `F1..F8` 并同步映射 delta；长度／槽位限制同步进 Schema。第二轮确认重提证明 `fact_ref` 错误归零，但五章均被 4 句 span 硬上限拒绝，因此最终将 4 句改为精度建议，仍严格检查 source ID 存在、首尾顺序、连续区间、白名单、关系、状态槽、重复与整体激活。最终 Backend 备份 `/opt/linoi/backups/20260805-173736`，现为 `61 legacy eligible / 10 partial / 0 active v2 revisions`；正文、旧记忆、305 条人物事件、296 条 legacy 状态与 20 个人物动态字段哈希不变。不得自动第三轮调用，11 个观察项不得执行。双端签名包、iOS 本机 IPA、macOS 换装完成；tag／Release 为 `v1.8.0-build31`。
