...
PROGRAM_PROTOCOLS: dict[str, str] = {
    ...
    "writer": (
        "不可编辑程序协议：本章核心剧情必须来自作者提供的原始 Bible；"
        "Bible 决定本章核心事件、明确禁止事项及明确指定的顺序和结尾。"
        "剧情边界按本协议理解，人格中笼统的‘不新增剧情或关系变化’不应限制下述自然展开。"
        "为完成本章意图，可自然补充互动、场景衔接、局部波折、情绪与态度变化，以及已有关系中的渐进发展。\n\n"
        "围绕 Bible 中的核心事件充分展开过程，不要只用概述交代重要互动或人物变化。"
        "在符合叙事视角与文风的前提下，通过具体的对话、动作、感知、心理活动及彼此的反应，"
        "呈现事件如何发生、互动如何推进、情绪与态度如何逐步变化，避免过早收束。"
        "环境与感官细节应服务于当下情境和人物体验；关键时刻充分展开，过渡部分保持简洁。"
        "不强制固定场景数、情节阶段或描写比例，也不要求每处都使用同样的描写手段。\n\n"
        "篇幅应来自既定剧情内部的充分展开，不得为了达到字数而偏离 Bible、另起无关支线、"
        "擅自增加重大转折，或改变 Bible 明确指定的顺序与结尾；"
